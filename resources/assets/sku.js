(function () {
    ...
    function SKU(warp, url, s) {
        this.url = url;
        this.s = $('select[name="' + s + '"]');
        this.warp = $(warp);
        this.attrs = {};
    ...
    SKU.prototype.getSkuList = function () {
        let that = this;
        this.s.on('change', function () {
            $.get(that.url + '&id=' + $(this).val(), function (res) {
                let html = '';
                let _default = $('.default-specifications');
                if (!res.length) {
                    _default.val('');
                    _default.removeAttr('data-id');
                    $('.tr-remove').remove();
                }
                $.each(res, function (index, val) {
                    if (index === 0) {
                        _default.val(val.name);
                        _default.attr('data-id', val.id);
                    } else {
                        html += '<tr class="tr-remove">' +
                            '<td><input type="text" class="form-control" data-id="' + val.id + '" value="' + val.name + '"></td>' +
                            '<td>' +
                ...
                that.warp.find('.sku_attr_key_val tbody').append(html);
            });
        });
    };
